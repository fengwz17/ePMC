...
import epmc.pomdp.model.Formulas;
...
public final class POMDP2JANIConverter {
    ...
    private final ModelPOMDP modelPOMDP;
    /** JANI model created. */
     ...
     * 
     * change modelPRISM to modelPOMDP
     * 
...
//        silentAction = modelJANI.getSilentAction();
    ...
    private void convertPlayers() {
        ...
        playerConverter.setPRISMModel(modelPOMDP);
    ...
    private void convertSystem() {
        ...
        systemConverter.setPRISMModel(modelPOMDP);
    ...
    private Constants buildConstants() {
    	String name = null;
        PropertiesImpl properties = modelPOMDP.getPropertyList();
        Constants janiConstants = new Constants();
        janiConstants.setModel(modelJANI);
        Formulas formulas = modelPOMDP.getFormulas();
    ...
    private JANIProperties buildProperties() {
        ...
        Properties oldProperties = modelPOMDP.getPropertyList();
        for (RawProperty raw : oldProperties.getRawProperties()) {
            Expression property = oldProperties.getParsedProperty(raw);
            if (forExporting) { 
                property = useOnlyPrefixedNamedRewards(modelPOMDP, property);
            }
     ...
     */
    private Expression useOnlyPrefixedNamedRewards(ModelPOMDP modelPOMDP, Expression expression) {
        assert expression != null;
        ...
        if (expression instanceof ExpressionReward) {
            RewardStructure reward = modelPOMDP.getReward(((ExpressionReward) expression).getReward());
            String name;
            if (reward == null) {
                name = "";
            } else {
            ...
            newChildren.add(new ExpressionIdentifierStandard.Builder()
                    ...
                    .build());
            newChildren.add(useOnlyPrefixedNamedRewards(modelPOMDP, oldChildren.get(1)));
            ...
            for (Expression child:oldChildren) {
                newChildren.add(useOnlyPrefixedNamedRewards(modelPOMDP, child));
            }
            return expression.replaceChildren(newChildren);
        }
    }
     ...
     * @return an equivalent expression using only named reward structures
     */
    public static Expression useOnlyNamedRewards(ModelPOMDP modelPOMDP, Expression expression) {
        assert expression != null;
        List<Expression> oldChildren = expression.getChildren();
        List<Expression> newChildren = new ArrayList<>(oldChildren.size());
        if (expression instanceof ExpressionReward) {
            ...
            newChildren.add(new ExpressionIdentifierStandard.Builder()
                    .setName(name)
                    .build());
            newChildren.add(useOnlyNamedRewards(modelPOMDP, oldChildren.get(1)));
            newChildren.add(oldChildren.get(2));
            newChildren.add(oldChildren.get(3));
            return expression.replaceChildren(newChildren);
        } else {
            for (Expression child:oldChildren) {
                newChildren.add(useOnlyNamedRewards(modelPOMDP, child));
            }
            return expression.replaceChildren(newChildren);
        }
    }
    ...
    private Expression useQuantitativePropertiesOnly(Expression expression) {
        ...
        if ((expressionQuantifier != null)
            ...
            if (dirType.equals(DirType.NONE)
                    && SemanticsNonDet.isNonDet(modelPOMDP.getSemantics())) {
    ...
    private InitialStates computeInitialStates() {
        ...
        Expression expInit = modelPOMDP.getInitialNodes();
        initialStates.setExp(expInit == null || !modelPOMDP.isMultipleInit()
                ? null : prism2jani(expInit));
        if (!modelPOMDP.isMultipleInit()) {
            return null;
        }
    ...
    private Variables buildGlobalVariables() {
        ...
        for (Entry<Expression, JANIType> entry : modelPOMDP.getGlobalVariables().entrySet()) {
            Expression varInit = modelPOMDP.getGlobalInitValues().get(entry.getKey());
